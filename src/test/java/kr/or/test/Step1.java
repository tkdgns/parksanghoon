 ...
 */
public class Step1 {
	//내부변수는 메서드 에서만 , 영향을 미치는 변수 = 메서드(함수)안에서만 사용하는 변수
	public static void main(String[] args) {
		//필드=메서드에 내부변수<->전역변수(맴버) 만들기(아래)
		...
		printMemeber(names,ages,phoneNums);//오버로드형 메서드 다향성 대표사례
 	}

	private static void printMemeber(String[] names, int[] ages, String[] phoneNums) {
		// member를 출력하는데 배열값을 받아서 구현(아래)
		int dataLength =names.length;//이름배열에 있는 사람의 명수구하기
	...
	private static void printMember(String name, int age, String phoneNum) {
		// member들 출력하는 프린트 메서드 구현 내용 (아래)
		System.out.println("회원의 이름은: "+name+" ㅣ 나이는: "+age+" ㅣ  연락처는: "+phoneNum);
	}
