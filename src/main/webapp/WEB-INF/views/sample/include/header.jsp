...
<html><!-- html문서는 태크열고 ~ 태그닫고 하는 영역확인 중요 -->
<head><!-- 문서내용과는 관계없는 문서정보가 들어있고, 메타데이터(데이터의데이터가 존재 -->
<meta charset="UTF-8"><!-- 문서의 언어 인코딩설정, 지금은 유니코드로 됨 -->
<!-- 반응형을 작동하기 위해서 사용하는 메타태그 뷰포트는 모니터,모바일액정,프린티-->
...
<style>
/*여기까지는 모바일용-아래부터는 반응형(태블릿,pc용)*/
/*pc용 스타일시작*/
/*미디어쿼리:미디어에 가로크기가 1132px 이상이면 아래스타일 적용*/
...
<!-- 상단 로고와 햄버거 메뉴영역 시작 -->
<h1 class="logo">
<a href="/">LOGO</a><!-- a태그는 페이지이동하는역할, href헤르프속성값에 URL을 입력해서 이동 -->
</h1><!-- 헤드라인 글자를 표시 h1~h6 -->
<!-- span 자리를 차지하지 않는 영역 = 한뼘 , px 픽셀 화면에 표시되는 크기에 단위-->
<!-- :클론(키:값,Key:Value) ;세미클론(문장의 끝) -->
...
<li><a href="javascript:alert('관리자단 준비중 입니다.');">AdminLTE</a>
</ul>
</nav>
<!-- 상단 로고와 햄버거 메뉴영역 끝 -->
</header>