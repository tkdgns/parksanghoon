#### 응용SW기초기술활용과목(4개의 단원)
- 스프링 MVC구조 중 controller 단 homecontroller.java 클래스를 분석해보겠습니다.
- 깃 사용하기 위해서 git scm 설치한 것처럼, cf(클라우드 파운드리) cli프로그램 설치가 필요합니다.
- 헤로쿠는 깃과 바로 연동되는 것이 장점.
- 파스타는 이클립스와 바로 연동되는 것이 장점.(1년에 1번은 뒤집어 엎어지기때문에, 여러분 자료가 다 사라집니다.)

-로그를 콘솔에 출력: Logger(클래스명) loigger(변수명) = 실행대상이 되는 클래스를 지정;
- 위처럼 변수를 선언하고, 로그를 사용하려고, logger 변수를 만들었습니다,
- 로그를
- public class HomController{ 내용 }
- 예를 들면, int(데이터타임) age(변수명) =30;
================================================================
- Homecontroller분석. MVC구조층 컨트롤러단 분석(아래)
- 클래스 특징: 클래스 이름은 반드시 대문자로 시작하는 규칙.
- Beans(=클래스): 스프링이 관리하는 클래스 빈(즈),
- 이름의 특징: 낙타등표기법=카멜 표기법을 사용합니다. HomeController
- 클래스의 MVC종류 역활 표시: 아래 3가지
- @controller : @Controller지시자 아래 클래스는 컨트롤러 역활이다. 명시
- @Service : @Service지시자 아래클래스는 프로그램직구현 역활이다라고 스프링에 명시
- @Repository :@Repository지시자는 데이터베이스 Model을 조작하는 역활이다라고 스프링에 명시.
- 참고로, html주석은: <!-- 주석내용 -->
- 자바주석: 2가지(아래)
- 1줄 주석: // 로 시작
- 여러줄 주석: /* ... */
...
- 개발자가 import 하는 것이 아니고, 이클립스가 자동으로 임포트 시켜줍니다.
- pavkage: 패키지(묶음, 짐보따리) 자바소스코드(클래스)를 관리하기 위해서 만든 영역(가상).http://controller.edu.org>org.edu.controller
- org.edu.controller 클래스용 초기에 프로젝트 생성할 때 만들었습니다.
- java폴더클래스루트입니다
- java.폴더안에.ogr폴더안controller폴더안쪽에 자바소스코드(자바클래스파일)이 생성됩니다.
- 자바클래스는 개발자가 만듭니다.(백앤드)
- 스프링 MVC구조 중  View단 home.jsp 화면 분석해 보았습니다.(프론트앤드)
- 프로트앤드에서 제작한 hrml디자인파일 개발자가 jsp파일로 프로그램을 입히게 됩니다.
- IT분야는 1번 했냐 ㅣ안했냐
- Java(자바): 오라클에서 제작한 ( 라이센스가 있는 ) 프로그래밍 언어.
================================================================
- 자바로 만든 홈페이지가 jsp 입니다.
- jsp(java server page): 스프링 MVC프로젝트 신규 생성( 새문서 생성) 시 
아래 기본출력
- 꺽쇠<% .... %>: jsp파일에서 자바프로그램을 사용할때 <% 자바프로그램 %>
- <%@ page 애노테이션 페이지 지시자(어) 역활: 페이지 인코딩(한글처리)
- <%@ taglib 애노테이션 태그라이브러리 지시자.
- prefix(접두사-축약어) ="C"
- <%@ page session 세션: 로그인 상태를 유지할때 사용합니다.
- 세션: 원래 웹페이지는 페이지 이동시 로그인(변수)정보가 사라집니다. <- 이런 상황을 방지하기위해서 세션(변수)를 쓰게 사용함.
- 기본은 session = "true" (세션변수를 사용하겠다는 의미이고 , 기본값입니다. )
- 태그: 의복(옷) 살표 가격표: 태그와 같은 역활 html정보 태그입니다.
- 가격표태그(정보), 타이틀태그(정보), 바디태크(본문내용정보), P태크(페러그래프-문단정보)
- <html><head></head><body><p>서버 시간...</body></html>
- 이클립스에서 단축기: 저장 [ctrl+s], 되돌리기[ctrl+z], 앞으로가기[ctrl+y]
- 영역지정 후 복사: [ctrl+c], 잘라내기 [ctrl+X]
- Deploy:배포
- taglib = url("외부자바프로그램용 태그를 가져다가 사용할 주소")
================================================================ 
- 시험 못보시면, 다음날 못보신분만 재시험, 오후 5시 40분 이후 1시간 보수 교육 이후 시험을 보시게 됩니다, 과제물 50점 서술형 50점
- 오후에는 스프링 MVC프로젝트의 구조확인하면서, 데이터전송에 대해서 설명드리고, 수정도 해 보겠습니다. 새문서 만들기처럼, 새 MVC프로젝트 만들때, 기본 생성되는 아래 파일2개 분석 (용어 설명) 후 수정에정
- home.jsp + Homecontroller.java
- 이클립스에서 글자크기 조정하기.
- 라이센스(저작권):아래
- GNU: GNU는 Not unlx이다. 유닉스Os 비싸기로 유명. 개발자가 거부감이 있어서 만든게 GNU라이센스입니다.
- LGPL: General Purpose License(일반목적의라이센스) 공개라이센스입니다.
- MIT라이센스: Mit 공대에서 만든 라이센스. 비영리,영리 상관없이 맘대로 갖다 쓸수 있는 제일 유연성있는 라이센스 입니다. 
- 1. 헤로쿠 에서는 Hsql 메모리데이터베이스를 사용.(서버를 껏다가 켜면, 내용이 원상복귀)
- 보통 솔루션은 데모페이지가 있습니다. 데모페이지에 자료를 등록이 가능합니다.
- 보통 데모페이지의 데이터는 서버를 저녁에 껐다가, 아침에 다시 키면, 데모페이지 데이터가 초기화됩니다.
- Hsql 데이터베이스가 위와같은 상황에서 사용됩니다.
- 2. 파스타에서는 mysql 데이터베이스사용.(서버를 껐다가 켜도, 내용이 게속 유지됨)
- 파스타는 다음주월요일 같이 진행하도록 하겠습니다.
- MVC: 자바개발 방식중에 3가지분리해서 개발하는방식 Model, view, controller
- Modl: 데이터베이스부분
- Controller: 데이터베이스(Modl)과 화면(view)단을 연결 시켜주는 역활.
- View: home.jsp 이것처럼, 화면부분 
####앞으로 여러분의 작업순서가 아래 처럼 진행됩니다.
- IDE: 통합개발환경 Intgration Development Environment( 이클립스, 안드로이드스튜디오)
- 앞으로 여러분들의 작업순서가 아래처럼 진행됩니다.
- log:로그 에러상황을 확일할때 보통 많이 사용하는 용어. log4j 사용해서 데이터전송상황, 에러표시 등을 사용하게됩니다
-1. 이클립스에서 학생이름의 프로젝트 소스수정+기능추가
-2. 이클립스수정한 결과, Run on sarver 로 톰켓에서 확인 OK, http://localhost:8080/
-3. 깃에푸쉬.
-4. 헤로쿠에서 Deloy/Branch(깃브랜지) 깃소스 -> 클라우드로 배포
-5. 헤로쿠에서 onenapp 버튼: http://학생명.herokuapp.com/
- pon.xml에서 수정사항이 발생되면,메이븐 업데이트가 필요합니다..
- 4단원: 개발환경 구축하기 시작
- 깃 올릴때 제외시킬 폴더를 지정, terget 폴더 깃에서 제외시키는법
- 아마존 웹 서비스(AMS)도 마찬가지, 클라우드 파스타와 마찬가지로, 헤로쿠도 클라우드 설정파일을 작성해야지만, 작동이 됩니다.
- 헤로쿠도 클라우드 설정파일=Profile
- 파일이름은 Procfile 파일명입니다.
- war(와르파일). Jgr(자르파일, java
-.gitignore:파일을 만들어서ㅡ target 이름을 적으시면됩니다.
- root루트: 최상의 위치를 가리키는 말.
- Aspect(관점): 
- 인젝트(inject-주사바늘삽입): 외부클래스 사용할때 쓰는 지시어 @inject 사용
- 이클립스 파일메뉴 -> 하단 REstart 클릭.
- 지시어 = 애노테이션= @
- 서블렛(servlet):server프로그램 let(조각)
